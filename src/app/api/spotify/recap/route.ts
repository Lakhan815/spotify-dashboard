...
export async function GET(request: Request) {
  ...
  const isFresh =
    existingRecap !== null &&
    new Date().getTime() - existingRecap.datePosted.getTime() < oneDayInMs;
  if (existingRecap && isFresh) {
    return NextResponse.json({
      tracksCaption: existingRecap.trackCaption,
      artistsCaption: existingRecap.artistCaption,
    });
  } else {
    ...
    try {
      ...
      await prisma.recap.create({
        data: {
          ...
          userId: user!.id,
          trackCaption: parsedData.tracksCaption,
          artistCaption: parsedData.artistsCaption,
          datePosted: new Date(),
        },
      });
